...
pub fn sanitize_stack(input: String) -> Vec<Vec<String>> {
    ...
    let parts: Vec<&str> = Regex::new(r"\s+")
        .unwrap()
        .split(&last.trim())
        .collect();
...
pub fn sanitize_instructions(input: String) -> Vec<Instruction> {
    ...
    for line in lines {
        ...
        output.push(
            Instruction { count: split[1].parse::<i32>().unwrap(), from: split[3].parse::<i32>().unwrap(), to: split[5].parse::<i32>().unwrap() }
        )
    }
...
pub fn day_five(input: Input, strategy: Strategy) -> Vec<Vec<String>> {
    ...
    for instruction in input.instructions {
        for _count in 0..instruction.count {
            ...
            match strategy {
                Strategy::PartOne => {
                    let mut value = output[from].drain(0..1).collect::<Vec<String>>();
...
mod tests {
    ...
    fn test_sanitize_instructions_day_five() {
        ...
        assert_eq!(output[0], Instruction {
            count: 1,
            from: 2,
            to: 1
        });

        assert_eq!(output[1], Instruction {
            count: 3,
            from: 1,
            to: 3
        });

        assert_eq!(output[3], Instruction {
            count: 1,
            from: 1,
            to: 2
        });
