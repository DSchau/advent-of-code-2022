...
pub fn get_input_as_array(input: String) -> Vec<Vec<String>> {
  ...
  for line in lines {
    let split_pair_collection = line.split(" ")
...
pub fn augment_strategy_array(input: Vec<Vec<String>>) -> Vec<Vec<String>> {
  ...
  for part in input {
    ...
    let left = opponent_lookup.get(&left_value as &str).unwrap_or(&"X").to_string();
    let right = &part[1];
...
pub fn get_score(input: Vec<Vec<String>>) -> i32 {
  ...
  let score_lookup = HashMap::from([
    ("X", 1),
    ("Y", 2),
    ("Z", 3)
  ]);

  let outcome_score_lookup = HashMap::from([
    ("WIN", 6),
    ("DRAW", 3),
    ("LOSE", 0)
  ]);
  ...
  for part in input {
    let left_value = &part[0];
    let left = &opponent_lookup.get(left_value as &str).as_deref().unwrap_or(&"X").to_string();
    let right = &part[1];
...
mod tests {
    ...
    fn test_strategy_arr() {
      ...
      assert_eq!(strategy_arr[0], vec!["A", "X"], "expect strategy to be draw, A = X"); // Draw, rock = rock
      assert_eq!(strategy_arr[1], vec!["B", "X"], "expect strategy to be loss, B > X"); // Lose, paper > rock
      assert_eq!(strategy_arr[2], vec!["C", "X"], "expect strategy to be win, C < X"); // Win, rock > scissors
    }
    ...
    fn test_strategy_arr_complex() {
      ...
      assert_eq!(strategy_arr[0], vec!["A", "X"], "expect strategy to be draw, A = X"); // Draw, rock = rock
      assert_eq!(strategy_arr[1], vec!["B", "Y"], "expect strategy to be draw, B = Y"); // Draw, paper = paper
      assert_eq!(strategy_arr[2], vec!["B", "Z"], "expect strategy to be win, B > X"); // Win, scissors > paper
    }
